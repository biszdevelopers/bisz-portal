...
export default defineEventHandler(async (event) => {
  ...
  if (member.role === "owner") return apiResult(event, new APIError("owner_protected", "The project owner cannot be removed", 409, "owner_protected"))
  await db.transaction(async (transaction) => {
    await transaction.delete(projectMembers).where(and(eq(projectMembers.projectId, projectId), eq(projectMembers.userId, userId)))
